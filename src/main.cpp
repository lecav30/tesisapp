...
bool createModel(const std::string& filename, Credentials& credentials, std::vector<Table>& tables, const char* directoryPath) {
    try {
        ...
        for (const auto& tableJson : tablesJson["tables"]) {
            ...
            std::ofstream file(std::string(directoryPath) + "/models/" + tableJson["name_const"].get<std::string>() + ".js");
            if (!file.is_open()) {
                std::cerr << "Error: No se pudo abrir el archivo de salida " + tableJson["name_const"].get<std::string>() + ".js'"
                          << std::endl;
            }
            file << result;
            file.close();
...
bool createController(std::vector<Table>& tables, const char* directoryPath) {
    try {
        ...
        for (const auto& tableJson : tablesJson) {
            ...
            std::ofstream file(std::string(directoryPath) + "/controllers/" + tableJson["name_const"].get<std::string>() + "Controller.js");
            if (!file.is_open()) {
                std::cerr << "Error: No se pudo abrir el archivo de salida " + tableJson["name_const"].get<std::string>() + "Controller.js'"
                          << std::endl;
            }
            file << result;
            file.close();
...
bool createRoute(std::vector<Table>& tables, const char* directoryPath) {
    try {
        ...
        for (const auto& tableJson : tablesJson["tables"]) {
            ...
            std::ofstream file(std::string(directoryPath) + "/routes/" + tableJson["name_const"].get<std::string>() + "Routes.js");
            if (!file.is_open()) {
                std::cerr << "Error: No se pudo abrir el archivo de salida " + tableJson["name_const"].get<std::string>() + "Routes.js'"
                          << std::endl;
            }
            file << result;
            file.close();
